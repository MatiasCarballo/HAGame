...
func spawn_fishes_from_user_data():
	var fishesData = TankData.getFish(UserData.user_data["tanks"]["Tank0"])#se puede obtener el id de c/d pez
	if fishesData.size() > 1:
		for fish_data in fishesData:
			...
			if fish_scenes.has(raza):
				...
				fish_instance.position = Vector2(randi_range(500, 700), randi_range(250, 450))
...
func spawn_fishes(raza):
	if fish_scenes.has(raza):
		var res: bool =await  TankData.addFish(UserData.user_data["tanks"]["Tank0"], "GoldenFish", "male")
		if res:
			var fish_instance = fish_scenes[raza].instantiate()
			fish_instance.position = Vector2(randi_range(500, 700), randi_range(250, 450))
			add_child(fish_instance)
